...
module.exports = function(object, options) {
  if (object.type === "Topology"){
    clockwiseTopology(object, options);
  } else {
    clockwiseGeometry(object, options);
  }
};
...
function clockwiseGeometry(object, options) {
  ...
  if (options && "coordinate-system" in options) {
  ...
  function clockwisePolygon(rings) {
    var r;
    if (rings.length && system.ringArea(r = rings[0]) < 0){
      r.reverse();
    }
    for (var i = 1, n = rings.length; i < n; ++i) {
      if (system.ringArea(r = rings[i]) > 0){
        r.reverse();
      }
    }
  }
...
function clockwiseTopology(topology, options) {
  ...
  function clockwisePolygon(rings) {
    var r;
    if (rings.length && ringArea(r = rings[0]) < 0){
      reverse(r);
    }
    for (var i = 1, n = rings.length; i < n; ++i) {
      if (ringArea(r = rings[i]) > 0){
        reverse(r);
      }
    }
  }
